...
public class CreateOrderServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            LocalDateTime dateCreate = ConvertConstant.convertStringtoLocalDateTime(dateCreateString);
            String notes = request.getParameter("notes");
            Service service = dao.getService(idService);
            TypeShift typeShift = dao.getTypeShift(CheckShift.checkHoliday(dateCreate));
            Shifts shifts = new Shifts(0, typeShift, typeShift.getCoefficient() * 120);
            dao.addShift(shifts);
            List<Shifts> shiftses = new ArrayList<>();
            
            shiftses.add(shifts);
            Order order = new Order(0, user, notes, "Pending", LocalDateTime.now(), shiftses);
            dao.addOrder(order);
