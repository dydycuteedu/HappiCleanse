...
public class ListServiceServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            Dao dao = new Dao();
            List<Service> listService = dao.getAllServices();
            List<Feedback> listFeedback = dao.getAllFeedbacks();
            request.setAttribute("serviceList", listService);
            request.setAttribute("feedbackList", listFeedback);
            request.getRequestDispatcher("ServiceList.jsp").forward(request, response);
        } catch (Exception ex) {
            Logger.getLogger(ListServiceServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
